...
def run(text: str, glossary_dict: Optional[dict[str, str]] = None) -> dict:
    """
    识别文本中的金融术语，返回术语表。
    ...
    # 将 glossary 精简为纯列表传给 Claude，节省 token
    glossary_keys = list(glossary_dict.keys())
    glossary_sample = glossary_keys[:300]  # 避免超长，取前300条做参考
...
参考词汇范围（部分）：{', '.join(glossary_sample[:50])}"""
    ...
    message = client.messages.create(
        model="claude-haiku-4-5-20251001",  # 前处理用轻量模型，节省成本
        max_tokens=512,
